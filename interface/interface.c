char *get_interface_name(CliEntry *entry){
    ...
    return cli.flag.get_str(name_flag,0,CLI_NOT_CASE_SENSITIVE);

}

char *get_new_name(CliEntry *entry){
    CliFlag *new_name_flag = cli.entry.get_flag(entry,"new_name | n ",CLI_NOT_CASE_SENSITIVE);
    ...
    return cli.flag.get_str(new_name_flag,0,CLI_NOT_CASE_SENSITIVE);
